...
class SystemVPN:
    def __init__(self):
        ...
        self.tun2socks_path = None
        self.running = False
        self.tap_adapter_name = None
    ...
    def setup_gui(self):
        ...
        info_label = tk.Label(
            info_frame,
            text="TÜM UYGULAMALAR + TÜM SİSTEM",
    ...
    def find_tap_adapter(self):
        try:
            ...
            for line in lines:
                if 'TAP-Windows' in line or 'TAP' in line or 'OpenVPN' in line or 'Wintun' in line:
                    parts = line.split()
                    if len(parts) >= 4:
            ...
            for line in result.stdout.split('\n'):
                if 'TAP' in line or 'OpenVPN' in line or 'Wintun' in line:
                    return line.strip()
    ...
    def download_tun2socks(self):
        if os.path.exists("tun2socks.exe"):
            return "tun2socks.exe"
            
        try:
            self.detail_label.config(text="Tun2socks indiriliyor...")
            
            url = "https://github.com/xjasonlyu/tun2socks/releases/download/v2.5.1/tun2socks-windows-amd64.zip"
            
            ssl._create_default_https_context = ssl._create_unverified_context
            urllib.request.urlretrieve(url, "tun2socks.zip")
            
            with zipfile.ZipFile("tun2socks.zip", 'r') as zip_ref:
                zip_ref.extractall(".")
                
            os.remove("tun2socks.zip")
            
            if os.path.exists("tun2socks-windows-amd64.exe"):
                os.rename("tun2socks-windows-amd64.exe", "tun2socks.exe")
    ...
    def setup_tun_interface(self):
        try:
            subprocess.run(
                f'netsh interface ip set address name="VPN-TUN" static 10.0.85.2 255.255.255.0',
                shell=True,
                capture_output=True
            )
            
            subprocess.run(
                f'netsh interface ip set dns name="VPN-TUN" static 8.8.8.8',
                shell=True,
                capture_output=True
            )
            
            subprocess.run(
                'route add 0.0.0.0 mask 0.0.0.0 10.0.85.1 metric 6',
                shell=True,
                capture_output=True
            )
    ...
    def start_vpn_thread(self):
        ...
        if not self.tun2socks_path:
            self.window.after(0, lambda: self.detail_label.config(text="Tun2socks kontrol ediliyor..."))
            self.tun2socks_path = self.download_tun2socks()
        ...
        try:
            ...
            self.window.after(0, lambda: self.detail_label.config(text="TUN interface başlatılıyor..."))
            
            if self.tun2socks_path and os.path.exists(self.tun2socks_path):
                try:
                    startupinfo = subprocess.STARTUPINFO()
                    startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                    startupinfo.wShowWindow = 0
                    ...
                    self.tun2socks_process = subprocess.Popen(
                        [self.tun2socks_path, '-device', 'tun://VPN-TUN', '-proxy', 'socks5://127.0.0.1:10808'],
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.DEVNULL,
                        startupinfo=startupinfo,
                        creationflags=subprocess.CREATE_NO_WINDOW
                    ...
                    self.window.after(0, lambda: self.detail_label.config(text="Sistem routingleri yapılandırılıyor..."))
                    time.sleep(1)
                    
                except Exception as tun_error:
                    self.window.after(0, lambda: self.detail_label.config(
                        text=f"TUN başlatılamadı (sadece tarayıcı modu): {str(tun_error)[:50]}"
                    ))
            ...
            mode = "TÜM SİSTEM" if self.tun2socks_process else "TARAYICI"
            
            self.window.after(0, lambda: self.update_status(
                "BAĞLI",
                f"VPN aktif - Mod: {mode} (Sistem genelinde çalışıyor)" if self.tun2socks_process else "VPN aktif - Sadece tarayıcılar",
                self.colors['success']
            ))
            ...
            self.window.after(0, lambda: self.show_notification(
                "VPN Bağlandı",
                "Tüm uygulamalar VPN üzerinden çalışıyor!" if self.tun2socks_process else "Sadece tarayıcılar VPN kullanıyor",
                success=True
            ))
    ...
    def stop_vpn(self):
        ...
        if self.tun2socks_process:
            try:
                ...
                time.sleep(1)
                if self.tun2socks_process.poll() is None:
